...
if size(Tracks{1},2) > 4
	error('more than 3 spatial dimensions')
end   

    for f = 1:length(FrameTracks)
		
		% get UNIQUE cell positions - keep the order stable
		[C, ia, ~] = unique(FrameTrackCoordinates{f}, 'rows', 'stable');
		...
		if length(C) ~= length(FrameTrackCoordinates{f})
			...
			for r = 1:length(removed)
				...
				doublette(end+1:end+length(find( FrameTrackCoordinates{f}(:,1) == FrameTrackCoordinates{f}(removed(r),1) & ...
					FrameTrackCoordinates{f}(:,2) == FrameTrackCoordinates{f}(removed(r),2)))) = find( FrameTrackCoordinates{f}(:,1) == FrameTrackCoordinates{f}(removed(r),1) & ...
